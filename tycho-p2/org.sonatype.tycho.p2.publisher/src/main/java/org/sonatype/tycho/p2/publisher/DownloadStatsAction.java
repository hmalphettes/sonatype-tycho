...
public class DownloadStatsAction extends AbstractPublisherAction {
	 ...
	 */
	public DownloadStatsAction(String statsURI, String statsTrackedBundles, String prefix, String suffix, boolean useArtifactVersion) {
		this(statsURI, split(statsTrackedBundles), prefix, suffix, true, useArtifactVersion, false);
	}
	...
	public DownloadStatsAction(String statsURI, Set<String> statsTrackedBundles,
		...
		this.statsSuffix = statsSuffix == null ? "" : statsSuffix;
		...
		for (String bundleId : statsTrackedBundles) {
			...
			for (IArtifactDescriptor artifact : artifacts.toArray(IArtifactDescriptor.class)) {
				if (artifact instanceof ArtifactDescriptor) {
					//this works for any IU: bundles and features
					((ArtifactDescriptor)artifact).setProperty("download.stats", getStatsValue(publisherInfo, artifact));
				}
			}
		}
		return Status.OK_STATUS;
